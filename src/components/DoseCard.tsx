...
export function DoseCard({
  ...
  const status = log?.status || statusForDose(dose, []);
  ...
  return (
    <Card className={`dose-card ${status || ""}`}>
      ...
      <div className="dose-actions">
        <Button onClick={() => onStatus(dose, "taken")} title="Mark this scheduled item as taken."><Check size={16} /> Taken</Button>
        <Button variant="ghost" onClick={() => onStatus(dose, "snoozed")} title="Snooze this item for later."><Clock3 size={16} /></Button>
        <Button variant="ghost" onClick={() => onStatus(dose, "skipped")} title="Mark this item as intentionally skipped."><SkipForward size={16} /></Button>
        <Button variant="ghost" onClick={() => onStatus(dose, "missed")} title="Mark this scheduled item as missed."><X size={16} /></Button>
      </div>
      {log && onUpdateLog && (
